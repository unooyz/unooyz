name = 'Jung kyoung mook'
age = 23 # not a lie, but i dnt't believe it lol, // if 23 has "" (look like "23") this is a word (don't have '' , the 23 is a number)
my_height = 182.5 #cm
my_weight = 75 #kg
my_eyes = 'black'
my_teeth = 'white'
my_hair=  'black'

print "Let's talk about %s." % name
print "He's %d centimeter tall" % my_height
print "He's %d kilogram heavy." % my_weight
print "Actually tha's not too heavy."
print "He's got %s eyes and %s hair." % (my_eyes, my_hair)
print "His teeth are uaually %s depending on the coffee." % my_teeth

# this live is tricky, try to get it exactly right
print "IF I add %d, %d, and %d I get %d." % (
       age, my_height, my_weight, age + my_height + my_weight)
